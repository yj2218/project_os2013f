...
/* Stack frame for kernel_thread() function. */
...
static bool isprior (const struct list_elem *, const struct list_elem *, void *);
...
for(;(!list_empty (&wait_list));) //만약 현재 tick이 이미 초과한 경우 
{
waitl=list_front (&wait_list);
wait_thread=list_entry(waitl,struct thread, elem);
if(curr < wait_thread -> time){break;}
else{
list_remove(waitl);
...
static bool
isprior(const struct list_elem *al, const struct list_elem *bl, void *aux UNUSED)
{
return list_entry (al,struct thread, elem)->time < list_entry (bl,struct thread, elem)->time;
}
...
The code provided sets the new thread's 'priority' to
...
again until awoken by thread_unblock() function.
...
curr->time = curr_tick + interval;
list_insert_ordered(&wait_list, &curr -> elem, isprior, NULL);
thread_block();
intr_set_level( old_level );
}
